...
class DeepspeedCheckpointHook(CheckpointHook):
    # the checkpoint hook used for deepspeed. The only diff is without @master_only
    def _save_checkpoint(self, runner):
        """Save the current checkpoint and delete unwanted checkpoint."""
        if 'deepspeed' in self.args.keys():
            del self.args['deepspeed']
        ...
        if runner.meta is not None:
            if self.by_epoch:
                cur_ckpt_filename = self.args.get(
                    'filename_tmpl', 'epoch_{}').format(runner.epoch + 1)
            else:
                cur_ckpt_filename = self.args.get(
                    'filename_tmpl', 'iter_{}').format(runner.iter + 1)
            runner.meta.setdefault('hook_msgs', dict())
            runner.meta['hook_msgs']['last_ckpt'] = self.file_client.join_path(
                self.out_dir, cur_ckpt_filename)
        # remove other checkpoints
        if self.max_keep_ckpts > 0 and runner.rank == 0:
            if self.by_epoch:
                name = 'epoch_{}'
                current_ckpt = runner.epoch + 1
            else:
            ...
            for _step in redundant_ckpts:
                ...
                if self.file_client.isfile(ckpt_path):
                    self.file_client.remove(ckpt_path)
                elif self.file_client.isdir(ckpt_path):
                    # os.rmdir(ckpt_path)
