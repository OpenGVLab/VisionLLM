...
class RandomFlip:
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            # if flip, the expression should also be modified
            if "expressions" in results:
                expressions = results["expressions"]
                flip_expressions = expressions.replace('left', '@').replace('right', 'left').replace('@', 'right')
                results["expressions"] = flip_expressions
        return results
