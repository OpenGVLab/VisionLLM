...
def clean_string(expression):
    expression = re.sub(r"([.,'!?\"()*#:;])", '', expression.lower()).replace('-', ' ').replace('/', ' ')
    return expression

def transform_expressions(expressions, clean=False):
...
class LoadImageFromFile:
    ...
    def __init__(self,
                 ...
                 clean_expression=True):
        self.to_float32 = to_float32
        self.color_type = color_type
        self.channel_order = channel_order
        self.file_client_args = file_client_args.copy()
        self.file_client = None
        self.clean_expression = clean_expression
    ...
    def __call__(self, results):
        ...
        ######################################################### 
        if 'expressions' in results['img_info']:
            results['expressions'] = transform_expressions(results['img_info']['expressions'], clean=self.clean_expression)
        if 'answer' in results['img_info']:
            results['answer'] = results['img_info']['answer']
        return results
