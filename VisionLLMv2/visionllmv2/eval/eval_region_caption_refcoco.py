...
class RefCOCOTest(RefCOCO):
    def preprocess_data(self, data_item):
        ...
        bboxes = data_item['gt_bboxes'].data[0].unsqueeze(0)  # [1, 4], xyxy in image shape (after aug), refcoco has 1 regions
        masks = data_item['gt_masks'].data[0].unsqueeze(0)    # [1, h, w], test only has 1 region
        ...
        # chat
        # question
        question_template = REFG_QUESTIONS[0] 
...
def eval_model(args):
    ...
    dataloader = DataLoader(dataset=dataset, sampler=sampler, collate_fn=custom_collate_fn,
                        batch_size=args.batch_size_per_gpu, num_workers=0)  
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ann_file", type=str, default="data/mdetr_annotations/refcocog_val_coco_format.json")
    parser.add_argument("--img_prefix", type=str, default="data/coco2014/train2014/")
