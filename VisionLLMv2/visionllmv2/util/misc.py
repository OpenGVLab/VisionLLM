...
def dict_to_cuda(input_dict, dtype=torch.bfloat16):
    for k, v in input_dict.items():
        if isinstance(input_dict[k], torch.Tensor):
            if isinstance(input_dict[k], torch.FloatTensor):
                input_dict[k] = v.cuda(non_blocking=True).to(dtype) 
            else:
            ...
            if isinstance(input_dict[k][0], torch.FloatTensor):
                input_dict[k] = [ele.cuda(non_blocking=True).to(dtype) for ele in v]
            else: 
