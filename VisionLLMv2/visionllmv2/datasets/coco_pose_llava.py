...
import visionllmv2.datasets.transforms.transform_coco as T
...
class CocoPoseLlavaDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, 
        ...
        super(CocoPoseLlavaDataset, self).__init__()
    ...
    def get_conversations(self, target=None):
        ...
        if self.image_set == 'train':
            random.shuffle(class_list)  # only 1 classes
...
class ConvertCocoPolysToMask(object):
    ...
    def __call__(self, image, target):
        ...
        keypoints = torch.as_tensor(keypoints, dtype=torch.float32).reshape(-1, 17, 3)  # [num_gt, 17, 3]
