...
# Dataset for loading UniKPT datasets, except for MP100 and Keypoint-5
# Because these two dataset have different kpt classes for differnt obj classes
import random
import torch
import copy
import os
...
import visionllmv2.datasets.transforms.transform_unikpt as T
...
class UniKPTLlavaDataset(torch.utils.data.Dataset):
    # NOTE: both CLASSES and POSE_CLASSES are loaded from ann_file
    # for some datasets (e.g. AP10K), there exist multiple classes
    ...
    def __init__(self, 
        ...
        super(UniKPTLlavaDataset, self).__init__()
        image_set = 'train' if not test_mode else 'val'
        self.image_set = image_set

        self.task = 'pose'
        self.dataset_name = 'unikpt'
        
        # evaluate
        self.eval_pck = eval_pck
        ...
        if image_set == 'train':
            ...
            for image_id in imgIds:
                ...
                # filter no kpt annotations
                if "num_keypoints" in target[0]:
        ...
        self.CLASSES = {
            cat['id']: clean_string(cat['name']) for cat in cats
        }
        # pose classes
        pose_cats = [cat['keypoints'] for cat in cats]
        if len(pose_cats) > 1:  # check keypoint classes are the same
            assert all(pose_cats[0] == sublist for sublist in pose_cats[1:])  
        pose_cats = cats[0]['keypoints']
        self.POSE_CLASSES = [clean_string(cat) for cat in pose_cats]
        self.num_classes = len(self.CLASSES)
        self.num_keypoints = len(self.POSE_CLASSES)
    ...
    def get_conversations(self, target=None):
        ...
        if self.image_set == 'train':
            ...
            if random.uniform(0, 1) < 0.2:  # all classes
                random.shuffle(class_list)
            else:  # keep all positives, random number negatives
                num_gt = len(target['labels'])
                ann_cat_labels = sorted(torch.unique(target['labels']).tolist())    # continuous ids
                ...
                max_num_neg = min(len(class_list), 5)
    ...
    def __getitem__(self, idx):
        ...
        img, target = self.prepare(img, target, num_keypoints=self.num_keypoints)
...
class ConvertCocoPolysToMask(object):
    ...
    def __call__(self, image, target, num_keypoints=17):
        w, h = image.size
        ...
        keypoints = torch.as_tensor(keypoints, dtype=torch.float32).reshape(len(anno), num_keypoints, 3)  # [num_gt, 17, 3]
...
def make_coco_transforms(image_set, args=None):
    ...
    if image_set == 'train':
        return T.Compose([
            # T.RandomHorizontalFlip(),  # since we do not know the flip_pairs
            T.RandomSelect(
