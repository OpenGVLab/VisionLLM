...
def crop(image, target, region):
    ...
    if "keypoints" in target:
        n_kpt = target['keypoints'].shape[1]
    ...
    if "keypoints" in target:
        ...
        target["keypoints"] = cropped_keypoints.view(target["keypoints"].shape[0], n_kpt, 3)  # NOTE: coco has 17 keypoints
...
def hflip(image, target):
    ...
    if "keypoints" in target:  # NOTE: coco flip pairs, idx start from 0
...
class Normalize(object):
    ...
    def __call__(self, image, target=None):
        ...
        # pad num_key_points=68
        if "keypoints" in target:
            keypoints = target["keypoints"]  # [num_gt, num_kpts, 3]
            V = keypoints[:, :, 2]           # [num_gt, num_kpts]
            V[V == 2] = 1
            Z=keypoints[:, :, :2]            # [num_gt, num_kpts, 2]
            ...
            target["keypoints"] = all_keypoints  # [num_gt, 68*3], xy in [0,1], xyxyzz
        return image, target
